        ...
        if (matches.length == 0) {
            ...
            for (let index = 0; index < matches.length; index++) {
                $scope.Coatingarray.push({
                    ...
                    product_description: matches[index].product_description,
                    product_code: matches[index].product_code,
                    option: matches[index].product_description.toUpperCase().replace($scope.Stock.toUpperCase(), '').replace($scope.Dimensions.toUpperCase(), '').replace('ROUND', '').replace('CORNER', '').replace('RC', '').replace('COVER', '').replace('BUSINESS', '').replace('CARDS', '').replace('CARD', '').replace('WITH', '').replace('18PT C1S', '').replace('100LB', '').replace('BC', '').replace('14PT', '').replace('LINEN', '').replace('32PT', '').replace('SADDLE STITCH CALENDAR ON', '').replace('CALENDAR ON', '').replace('-', '').replace('#', '').replace('FLYERS', '').replace('FLYER', '').replace('SOCIAL', '').replace('POST', '').replace('"', '')
                })
                $scope.$apply()
            }
